 ...
 */
 private boolean wrap(String filename, boolean inValidationMode){
  try {
   ...
   parser.setErrorHandler(new trencadis.infrastructure.services.KeyServer.impl.wrapper.config.CustomErrorHandler());
...
/**
 * Elimina el elemento DOMAIN y sus descendientes
 ...
 */
 public void del_DOMAIN(){
 try{
  Element e = getElementByTagName("DOMAIN");
...
/**
 * Elimina el elemento IDENTITY y sus descendientes
 * @throws java.util.NoSuchElementException Si el atributo no se
 *           encuentra
 * @throws IllegalStateException Si el schema impide eliminar m�s elementos
 */
 public void del_IDENTITY(){
 try{
  Element e = getElementByTagName("IDENTITY");
  e.getParentNode().removeChild(e);
  }catch(Exception ex){ throw new java.util.NoSuchElementException(" Se ha intentado borrar un elemento inexistente");}
 }
