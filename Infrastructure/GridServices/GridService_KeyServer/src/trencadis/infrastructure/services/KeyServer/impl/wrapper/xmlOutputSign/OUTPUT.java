package trencadis.infrastructure.services.KeyServer.impl.wrapper.xmlOutputSign;
...
/**
 * Obtiene el valor del elemento SIGNEDMESSAGE
 * @return El valor del elemento SIGNEDMESSAGE
 */
public String get_SIGNEDMESSAGE(){
 try {
 String unparsedValue = getValueByTagName("SIGNEDMESSAGE");
 String parsedValue = unparsedValue;
 return parsedValue;
 }catch (Exception e) { e.printStackTrace(); return null;}
}
...
/**
 * Asocia un valor al elemento SIGNEDMESSAGE
 * Si el elemento no existe, lo crea previamente
 * @param value El valor a establecer al elemento SIGNEDMESSAGE
 * @throws IllegalArgumentException Si el valor incumple alguna
 *         de las restricciones del schema
 */
public void set_SIGNEDMESSAGE(String value){
 String unparsedValue = value;
 try{
 setValueByTagName("SIGNEDMESSAGE", unparsedValue);
 } catch(Exception e) { //Si no existe el elemento lo creamos
   Element el = doc.createElement("SIGNEDMESSAGE");
   el.appendChild(doc.createTextNode(unparsedValue));
   elem.appendChild(el);
 }
}
